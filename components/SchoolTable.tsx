...
export default function SchoolTable({ data }: SchoolTableProps) {
  ...
  const schoolOptions = useMemo(
    () => ["All", ...new Set(data.map((s) => s.name))],
    [data]
  );
  const districtOptions = useMemo(
    () => ["All", ...new Set(data.map((s) => s.district || ""))],
    [data]
  );
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredSchools.map((school) => (
              ...
              >
                ...
                <td className="px-6 py-4">{school.staff ?? "-"}</td>
      ...
      {filteredSchools.length > 0 && (
        <div className="bg-gray-50 px-6 py-3 border-t border-gray-200 text-xs text-gray-500">
          Showing {filteredSchools.length} of {data.length} schools
        </div>
      )}
    </div>
  );
